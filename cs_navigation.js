...
s.src = chrome.extension.getURL('inj_input.js');
s.onload = function() {
    s.remove();
};
...
function check(description, communicationRegime) {
    ...
    console.log("Bowling: already waiting for " + checkElapsedTime + " ms");
    if ($(targetSelector).length >= 1 || property === "presence") {
        console.log("Bowling: performing check");
        switch (property) {
            case "presence":
            case "count":
        ...
        console.log("Bowling: error when checking - can't find element");
...
function clickElement(description, causesReload, reloadType, communicationRegime) {
    ...
    if (($(selector).length > 0) && (String($(selector).css("display")) !== "none")) {
        ...
        //console.log("Bowling: " + $(selector).length + " elements to click on");
        //console.log("Bowling: " + "css display: " + String($(selector).css("display")));
        reportNavigationStatus(communicationRegime, "error");
    }
}
...
chrome.runtime.onConnect.addListener(function(port) {
    if (port.name === "navigationPort") {
        port.onMessage.addListener(function(request) {
            console.log("Bowling: performing " + request.purpose + ", doing " + request.action + "'ing");
            ...
            function respondToUserActionRequest() {
                console.log("Bowling: wait " + Math.max(originalDelay - (Date.now() - subtaskStarted), minimumDelay) + "ms more");
            ...
            if (request.purpose === "navigation") {
                switch (request.action) {
                    case "scroll":
                        ...
                        if (request.description.showStopper === "telegramChannel") {
                            scrollPage(request.description, "content-background", tlgrmChnlStopScrollingCondition);
                        }
                        if (request.description.showStopper === "telegramChannelFull") {
                            scrollPage(request.description, "content-background", tlgrmFullChnlStopScrollingCondition);
                        }
                        break;
                    case "click":
                        setTimeout(function() {
