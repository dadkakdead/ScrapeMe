    ...
    if (changeInfo.status === "complete") {
        if ((getWkr(tabId) !== null) && (getWkr(tabId).pendingTabUpdate === true)) {
            ...
            if (getWkr(tabId).getStatus() === "not started") {
                console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " starting");
            }
    ...
    if (tasksManager(taskName).isEmpty() === false) {
        chrome.storage.local.get(["scraperParameters"], function(storageResponse) {
            ...
            chrome.windows.create({url: chrome.extension.getURL('bg.html'), incognito: incognitoMode}, function(window){
                console.log("Bowling Log: " + "-" + taskName + "-" + " initializing");
                windowId = window.id;
...
function runTask(tabId) {
    ...
    if ((getWkr(tabId).targetPages === "rootPageOnly" || linksMotivatedTask) && (getWkr(tabId).includesSearchAction() === false || searchMotivatedTask)) {
        ...
        if (getWkr(tabId).suppressReloadBetweenTasks === false) {
            console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " loading root page");
        ...
        console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " is finished");
...
function continueTask(tabId) {
    ...
    if (taskStatus === "finished") {
        if (getWkr(tabId).isComplex() === true) {
            ...
            console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " starting again");
    ...
    if (taskStatus === "in progress") {
        console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " continues");
...
function navigationUtility(tabId) {
    console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " performing navigation subtask");
    chrome.webNavigation.getAllFrames({tabId: tabId}, function(framesList){
        ...
        if (frameId !== null) {
            ...
            port1.onMessage.addListener(function(request) {
                console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " processing message from tab " + tabId);
                ...
                if (request.purpose === "eventTriggering") {
                    console.log("Bowling Log: " + "bump bump");
                }
            });

            //disconnection event states for end of navigation
            port1.onDisconnect.addListener(function() {
                console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " navigation subtask finished");
                if (getWkr(tabId).pageCurrentSubtask.status === "in progress") {
                    console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " navigation ok, go next");
                    getWkr(tabId).pageCurrentSubtask.status = "finished";
                    ...
                    if (getWkr(tabId).pageCurrentSubtask.causesReload === true) {
                        ...
                        console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " continue after navigation, don't wait for loading");
                        continueTask(tabId);
                    }
                }
                if (getWkr(tabId).pageCurrentSubtask.status === "error") {
                    console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " " + getWkr(tabId).pageCurrentSubtask.errorHandling + " error during navigation");
            ...
            console.log("Bowling Log: " + "-" + getWkr(tabId).taskName + "-" + " frame not found, restarting");
            restartTask(tabId);
        }
    });
}
...
function scrapingUtility(tabId) {
    console.log("Bowling: " + "-" + getWkr(tabId).taskName + "-" + " performing scraping");
    ...
    //listen to incoming scraping data
...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    switch (request.controllerCommand) {
        case 'log':
            console.log("Bowling Log: " + request.data);
        ...
        case "initTask":
            if (taskNames.indexOf(request.taskName) > -1) {
                        ...
                        chrome.storage.local.get(["scraperParameters", "linksVault", "searchTasksVault", "workersVault"], function(sustainableCache) {
                            chrome.storage.local.clear(function() {
                                chrome.storage.local.set(sustainableCache, function() {
                                    exportListLength = 0;
                                    console.log("Bowling Log: " + "export cache erased");
                ...
                console.log("Bowling Log: " + " couldn't find task with name " + request.taskName);
            }
            break;
