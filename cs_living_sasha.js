//boolean to allow only single review() call
var botActivated = false;
var botBirthdayTimestamp = Date.now();
var botBirthdayMidnight = sameDayMidnight(new Date());

var dialoguesCheckInterval = 200;

//counter of marked dialogues
var dialoguesMarkerCounter = 0;
//counter of read dialogues
var dialoguesReaderCounter = 0;

var timeToCollect = 25;

//read all the messages from previous visit (50 messages max)
var maxMessagesForSasha = 20;
var scrolledToBeginningOfChannel;
...
var tlgrmFullChnlStopScrollingCondition = function(scrollingFunction, communicationRegime) {
    ...
    //filter for flood channels
    messagesFrequency = 0;
    if (capturedPeriodInDays > 2) {
        messagesFrequency = nMessagesLoadedCounter / capturedPeriodInDays;
        if (messagesFrequency > 7) {
            return reportNavigationStatus(communicationRegime, "error");
        }
    }

    console.log("Bowling Stats: First message from " +  firstMessageLoadedDate);
    console.log("Bowling Stats: Last message from " + lastMessageLoadedDate);
    console.log("Bowling Stats: Loaded " + nMessagesLoadedCounter + " messages");
    console.log("Bowling Stats: Captured range " + capturedPeriodInDays + " days; " + messagesFrequency + " messages per day");
...
var tlgrmChnlStopScrollingCondition = function(scrollingFunction, communicationRegime) {
    ...
    if (nMessages + nServiceMessages > 0) {
        ...
        if (firstMessageDate !== undefined && firstMessageTime !== undefined && lastMessageDate !== undefined && lastMessageTime !== undefined) {
            ...
            console.log("Bowling info: Loaded " + nMessages + " messages");
            console.log("Bowling info: The oldest post loaded " + firstMessageTimeFull + ", " + dtDaysFromOldestPostLegible + " days ago");
            console.log("Bowling info: Last post on " + lastMessageTimeFull + ", " + dtDaysFromLastPostLegible + " days ago");
            console.log("Bowling info: Range " + dtDaysRangeLegible + " days");
